...
def getDeployTargets(envName) {
  ...
  targets['master'] = [[
    ...
    COPY_DIR: '/mnt/blockstorage/containers/telegram_validator'
  ]]
...
pipeline {
  ...
  environment {
    ...
    DOCKER_IMAGE_NAME = 'sjc.vultrcr.com/dongregistry/oauth-validator'
  ...
  stages {
    stage('체크아웃') {
      steps {
        ...
        git branch: BUILD_BRANCH, credentialsId: GIT_KEY_ID, url: 'git@github.com:donghquinn/java-oauth-validator.git'
      }
    }
